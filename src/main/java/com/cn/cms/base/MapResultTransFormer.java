...
public class MapResultTransFormer extends BasicTransformerAdapter implements Serializable {
     ...
     * 例如: 当构造参数为List("userName","userId")时,对于SQL查询:
     * <code> select username,userID from table</code> 返回字段名称列表为userName,userId<br/>
     ...
     * 例如: 当构造参数为Map("userName":"name","userId":"id")时,对于SQL查询:
     * <code> select username,userID from table</code> 返回字段名称列表为name,id<br/>
